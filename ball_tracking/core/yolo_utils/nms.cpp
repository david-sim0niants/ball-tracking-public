...
inline double interval_overlap(double a0, double b0, double a1, double b1)
{
    ...
    else
    {
        if (b0 < a1)
            return 0;
        else return std::min(b0, b1) - a1;
    }   
}

inline double bbox_iou(BBox bbox_a, BBox bbox_b)
{
    double intersect_w = interval_overlap(bbox_a.x, bbox_a.x + bbox_a.w, bbox_b.x, bbox_b.x + bbox_b.w);
    double intersect_h = interval_overlap(bbox_a.y, bbox_a.y + bbox_a.h, bbox_b.y, bbox_b.y + bbox_b.h);
    
    double intersect = intersect_w * intersect_h;
    double _union = bbox_a.w * bbox_a.h + bbox_b.w * bbox_b.h - intersect;
...
void NonMaximumSuppression(std::vector<BBox> &bboxes, std::vector<Classes> &classes_arr, double thresh)
{
    ...
    {
        std::vector<double> classes_along_bboxes(bboxes.size());
